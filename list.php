...
<?php
    ...
    $nomal_query = empty($search_title) && empty($search_name) && empty($fir_date) && empty($sec_date) ? con_query("select * from information $order_by LIMIT $start , $count_per_page") : '' ;

    $search_title_query = empty($search_title) ? '' : con_query("SELECT * FROM information WHERE info_title LIKE CONCAT('%','$search_title','%') $order_by LIMIT $start , $count_per_page");
    
    $search_name_query = empty($search_name) ? '' : con_query("SELECT * FROM information WHERE info_title LIKE CONCAT('%','$search_name','%') $order_by LIMIT $start , $count_per_page");
    
    $search_date_query = empty($fir_date) && empty($sec_date) ? '' : con_query("SELECT * FROM information WHERE info_date >= '$fir_date' AND info_date <= '$sec_date' $order_by LIMIT $start , $count_per_page");
    
    $search_dual_query = empty($search_title) && empty($search_name) ? '' : con_query("SELECT * FROM information WHERE info_title LIKE CONCAT('%','$search_title','%') and info_name LIKE CONCAT('%','$search_name','%') $order_by LIMIT $start , $count_per_page");
    
    $search_quad_query = empty($search_title) && empty($search_name) && empty($fir_date) && empty($sec_date) ? '' : con_query("SELECT * FROM information WHERE info_title LIKE CONCAT('%','$search_title','%') and info_name LIKE CONCAT('%','$search_name','%') and info_date >= '$fir_date' AND info_date <= '$sec_date' $order_by LIMIT $start , $count_per_page");
...
<?php
    ...
    while ($roww = mysqli_fetch_array($search_query_select)) {
    
        echo '<tr>';
        echo '<td class="border_other">'.$roww[0].'</td>';
        echo '<td class="border_other">'.$roww[2].'</td>';
        echo '<td class="border_title"><a href="read.php?list_num='.$roww['0'].'">'.$roww[5].'</a></td>';
        
        $file_check = con_query("select file_name from file_manager where num='".$roww['info_num']."'");
        
        if(mysqli_fetch_assoc($file_check) != NULL){
            echo '<td class="border_other"><img src="img/yesFile.png"></td>';
        }else
            echo '<td class="border_other"></td>';
        echo '<td class="border_other">'.$roww[7].'</td>';
        echo '<td class="border_other">'.$roww[1].'</td>';
        echo '<td class="border_other">'.$roww[8].'</td>';
        echo '</tr>';
    }
?>
		</table>
...
/* 				    echo '<input type="submit" id="page'.$i.'" value="'.$i.'" onclick="location.href=\'list.php?list_page='.$i.'&?search_title='.$search_title.'&?search_name='.$search_name.'&?search_date_fir='.$fir_date.'&?search_date_sec='.$sec_date.'\'">'; */
		...
		<form action="" method="get">
        	<div id="LS">
        		<span>제목</span> <input type="text" id ="search_title" name="search_title">
        		<span>작성자</span> <input type="text" id="search_name" name="search_name">
        		<span>작성일</span> <input id="firDate" type="date" id="search_date_fir" name="search_date_fir">
        		<span>~</span> <input id="secDate" type="date" id="search_date_sec" name="search_date_sec">
        		<button type="submit">검색</button>
        	</div>
        </form>
